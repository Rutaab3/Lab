...
if (isset($_GET['action'], $_GET['id'])) {
  ...
  // Fetch supplier info for log
  $res = mysqli_query($conn, "SELECT username FROM users WHERE id=$supplierId AND role='supplier'");
  $supplier = mysqli_fetch_assoc($res);

  if ($supplier) {
    if ($action === 'ban') {
      mysqli_query($conn, "UPDATE users SET status='banned' WHERE id=$supplierId");
      log_action($conn, "Supplier Banned", ['target_user_id' => $supplierId, 'target_username' => $supplier['username']]);
      $msg = "Supplier Banned";
    } elseif ($action === 'unban') {
      mysqli_query($conn, "UPDATE users SET status='active' WHERE id=$supplierId");
      log_action($conn, "Supplier Unbanned", ['target_user_id' => $supplierId, 'target_username' => $supplier['username']]);
      $msg = "Supplier Unbanned";
    } elseif ($action === 'demote') {
      mysqli_query($conn, "UPDATE users SET role='user' WHERE id=$supplierId");
      log_action($conn, "Supplier Demoted", ['target_user_id' => $supplierId, 'target_username' => $supplier['username']]);
      $msg = "Supplier Demoted";
    }
  }

  header("Location: manage_suppliers.php?msg=" . urlencode($msg));
  exit();
}


// --- Fetch suppliers ---
$supplier = mysqli_query($conn, "SELECT id, username, email, profile_img, status, created_at FROM users WHERE role='supplier' ORDER BY created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<td>
            ...
            <td>
              <?php if ($t['status'] === 'active'): ?>
                <a href="#" onclick="confirmAction(event, '?action=ban&id=<?= $t['id'] ?>', 'ban this supplier')" class="btn-delete">
                  <i class="bi bi-slash-circle"></i> Ban
                </a>
              <?php else: ?>
                <a href="#" onclick="confirmAction(event, '?action=unban&id=<?= $t['id'] ?>', 'unban this supplier')" class="btn-update">
                  <i class="bi bi-check-circle"></i> Unban
                </a>
              <?php endif; ?>

              <a href="#" onclick="confirmAction(event, '?action=demote&id=<?= $t['id'] ?>', 'demote this supplier to user')" class="btn-delete">
